    ...
    <main class="govuk-main-wrapper" id="main-content" data-testid="event-main">
        <?php if (have_posts()) : while (have_posts()) : the_post(); ?>
            ...
            <div class="event-details" data-testid="event-details">
                <?php 
                $end_raw   = get_field('end_datetime');

                $start_ts = strtotime(get_field('start_datetime'));
                $end_ts   = $end_raw ? strtotime(get_field('end_datetime')) : false;

                $start_date = date('j F Y', $start_ts);
                $start_time = date('g:i a', $start_ts);
                ...
                if (!$end_ts) {
                    ...
                    $end_date = date('j F Y', $end_ts);
                    $end_time = date('g:i a', $end_ts);

                    $date = $start_date === $end_date ? $date = "{$start_date}" : $date = "{$start_date} to {$end_date}";
