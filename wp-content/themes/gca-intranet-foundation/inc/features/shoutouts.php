...
if (class_exists('GFForms')) {
    ...
    // ---------------------------------------------------------------------------
    // Gravity Forms – Shoutout Compose form (rendered in community wall)
    // ---------------------------------------------------------------------------

    add_action('init', 'gca_shoutout_maybe_create_compose_form');

    function gca_shoutout_maybe_create_compose_form(): void
    {
        if (!class_exists('GFAPI')) {
            return;
        }

        $stored = (int) get_option('gca_shoutout_compose_form_id', 0);
        if ($stored > 0 && GFAPI::form_id_exists($stored)) {
            return;
        }

        $form_id = GFAPI::add_form([
            'title'        => 'Shout-out',
            'button'       => ['type' => 'text', 'text' => 'Post shout-out'],
            'requireLogin' => true,
            'fields'       => [
                [
                    'id'          => 1,
                    'type'        => 'text',
                    'label'       => 'Who are you shouting out?',
                    'placeholder' => 'Search for a colleague…',
                    'isRequired'  => true,
                    'cssClass'    => 'gca-shoutout-gf-field--recipient',
                ],
                [
                    'id'    => 2,
                    'type'  => 'hidden',
                ...
                [
                    ...
                    'label'      => 'Category',
                    'isRequired' => true,
                    'cssClass'   => 'gca-shoutout-gf-field--category',
                    'choices'    => [['text' => 'Select a category', 'value' => '']],
                ],
                [
                    'id'          => 4,
                    'type'        => 'textarea',
                    ...
                    'isRequired'  => true,
                    'maxLength'   => 500,
                    'cssClass'    => 'gca-shoutout-gf-field--message',
                ],
            ],
            'confirmations' => [
                [
                    ...
                    'isDefault' => true,
                    'type'      => 'message',
                    'message'   => 'Shout-out posted!',
                ],
            ],
        ]);

        if (!is_wp_error($form_id)) {
            update_option('gca_shoutout_compose_form_id', (int) $form_id);
        }
    }

    // Populate category choices dynamically before rendering the compose form
    add_filter('gform_pre_render', function (array $form): array {
        $compose_id = (int) get_option('gca_shoutout_compose_form_id', 0);
        if (!$compose_id || (int) $form['id'] !== $compose_id) {
            return $form;
        }
        ...
        foreach ($form['fields'] as $field) {
            ...
            $choices = [['text' => 'Select a category', 'value' => '', 'isSelected' => true]];
            foreach ($terms as $term) {
                $choices[] = ['text' => $term->name, 'value' => (string) $term->term_id, 'isSelected' => false];
            }
