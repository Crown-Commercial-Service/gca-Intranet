...
(function () {
    ...
    function confirmDelete(triggerEl, title, body) {
        return new Promise(function (resolve) {
            ...
            modal.innerHTML =
                ...
                '<div class="gca-lc-delete-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="gca-poll-delete-title" aria-describedby="gca-poll-delete-description" tabindex="-1">' +
                ...
                '<h2 class="gca-lc-delete-modal__title" id="gca-poll-delete-title">' + esc(title) + '</h2>' +
                '<p class="gca-lc-delete-modal__body" id="gca-poll-delete-description">' + esc(body) + '</p>' +
    ...
    function setupPollMenus(containerEl) {
        containerEl.addEventListener('click', function (e) {
            ...
            confirmDelete(deleteBtn, 'Are you sure you want to delete this poll?', 'This will permanently delete the poll. You cannot undo this action.')
                .then(function (confirmed) {
                    if (!confirmed) { return; }
                    var positions = [];
                    document.querySelectorAll('#gca-poll-' + pollId).forEach(function (el) {
                        positions.push({ el: el, parent: el.parentNode, next: el.nextSibling });
                        el.remove();
                    });
                    if (window.gcaShowDeleteToast) { window.gcaShowDeleteToast('Poll deleted successfully.'); }
            ...
            .finally(function () {
                ...
                if (page === 1 && window.location.hash) {
                    setTimeout(function() {
                        ...
                        try {
                            var panel = document.getElementById('gca-panel-polls');
                            ...
                            if (target) {
                                ...
                                setTimeout(function() {
                                    ...
                                    console.log("Polls JS: scrolled to Y:", targetY);
                                }, 10);
