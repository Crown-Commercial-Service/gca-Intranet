...
class core_google_apps_login {
	...
	public function ga_plugins_loaded() {
		load_plugin_textdomain( 'googleappslogin-enterprise', false, dirname($this->my_plugin_basename()).'/lang/' );
	}
	...
	public function ga_login_form() {
		...
		if ($do_autologin && $options['ga_clientid'] != '' && $options['ga_clientsecret'] != '') {
			if (!headers_sent()) {
				...
				<p><b><?php printf( __( 'Redirecting to <a href="%s">Login via Google</a>...' , 'googleappslogin-enterprise'), $authUrl ); ?></b></p>
	...
	if (isset($options['ga_disableGlogin']) && $options['ga_disableGlogin']!=1) {
		...
		if(isset($options['btn_google_signin_image']) && !empty($options['btn_google_signin_image']) && $options['btn_google_signin_image'] != 'custom_text'){
			...
			<span class="google-apps-header '.$loginWithGoogleImage.'">
                <span class="inner">
					...
					$LoginWithGoogleButton = '<a href="'.$authUrl.'"><img src="'.$this->my_plugin_url().'img/'.$loginWithGoogleImage.'.png" /></a>';
				}
				?>
				...
				<p class='galogin-powered'><?php esc_html_e( 'Powered by ' , 'googleappslogin-enterprise'); ?><a href='https://wp-glogin.com/?utm_source=Login%20Form&utm_medium=freemium&utm_campaign=LoginForm' target="_blank">wp-glogin.com</a></p>
				<?php } ?>
				...
				jQuery(document).ready(function(){
					...
					?>
						loginform.prepend("<h3 class='galogin-or'><?php esc_html_e( 'or' , 'googleappslogin-enterprise'); ?></h3>");
					<?php } ?>
...
<?php }
	...
	protected function get_login_button_text() {
		$login_button_text = __('Login with Google', 'googleappslogin-enterprise');
	...
	public function ga_authenticate($user, $username=null, $password=null) {
		if (isset($_REQUEST['error'])) {
			switch ($_REQUEST['error']) {
				case 'access_denied':
					$error_message = __( 'You did not grant access' , 'googleappslogin-enterprise');
				break;
				case 'ga_needs_configuring':
					$error_message = __( 'The admin needs to configure Google Apps Login plugin - please follow '
										...
										.' target="_blank">instructions here</a>' , 'googleappslogin-enterprise');
				break;
				case 'ga_user_must_glogin':
					$error_message = sprintf(__( 'The user must use <i>%s</i> to access the site' , 'googleappslogin-enterprise'),
					                        htmlentities($this->get_login_button_text()));
				break;
				default:
					$error_message = __( 'Unrecognized error message' , 'googleappslogin-enterprise');
				break;
			}
		...
		if (isset($_GET['code'])) {
			if (!isset($_REQUEST['state'])) {
				$user = new WP_Error('ga_login_error', __( "Session mismatch - try again, but there could be a problem setting state" , 'googleappslogin-enterprise') );
				return $this->displayAndReturnError($user);
			}

			$statevars = explode('|', urldecode($_REQUEST['state']));
			if (count($statevars) != 2) {
				$user = new WP_Error('ga_login_error', __( "Session mismatch - try again, but there could be a problem passing state" , 'googleappslogin-enterprise') );
				return $this->displayAndReturnError($user);
			}
			...
			if (!$this->session_indep_verify_nonce($retnonce, 'google_apps_login-'.$this->get_cookie_value())) {
				$user = new WP_Error('ga_login_error', __( "Session mismatch - try again, but there could be a problem setting cookies" , 'googleappslogin-enterprise') );
				return $this->displayAndReturnError($user);
			}
			...
			try {
				...
				if ($userinfo && is_object($userinfo) && property_exists($userinfo, 'email')
					...
					if (!$google_verified_email) {
						$user = new WP_Error('ga_login_error', __( 'Email needs to be verified on your Google Account' , 'googleappslogin-enterprise') );
					}
					else {
					...
					$user = new WP_Error('ga_login_error', __( "User authenticated OK, but error fetching user details from Google" , 'googleappslogin-enterprise') );
				}
	...
	protected function createUserOrError($userinfo, $options) {
		return( new WP_Error('ga_login_error', sprintf( __( 'User %s not registered in WordPress' , 'googleappslogin-enterprise'),
												 $userinfo->email) ) );
	}
	...
	public function ga_admin_auth_message() {
		...
		echo sprintf( __('You will need to complete Google Apps Login <a href="%s">Settings</a> in order for the plugin to work', 'googleappslogin-enterprise'),
	...
	public function ga_admin_menu() {
		if (is_multisite()) {
			add_submenu_page( 'settings.php', __( 'Google Apps Login settings' , 'googleappslogin-enterprise'), __( 'Google Apps Login' , 'googleappslogin-enterprise'),
				'manage_network_options', $this->get_options_menuname(),
				array($this, 'ga_options_do_page'));
		}
		else {
			add_options_page( __( 'Google Apps Login settings' , 'googleappslogin-enterprise'), __( 'Google Apps Login' , 'googleappslogin-enterprise'),
	...
	public function ga_options_do_page() {
		...
		<h2><?php _e('Google Apps Login setup', 'googleappslogin-enterprise'); ?></h2>

		<div id="gal-tablewrapper">

		<div id="gal-tableleft" class="gal-tablecell">

		<p><?php _e( 'To set up your website to enable Google logins, you will need to follow instructions specific to your website.', 'googleappslogin-enterprise'); ?></p>
		...
		_e( 'Click here to open your personalized instructions in a new window' , 'googleappslogin-enterprise'); ?></a></p>
		...
		<p class="submit">
			<input type="submit" value="<?php esc_attr_e( 'Save Changes' , 'googleappslogin-enterprise'); ?>" class="button button-primary" id="submit" name="submit">
		</p>
		</form>
		</div>
	...
	protected function ga_options_do_network_errors() {
		if (isset($_REQUEST['updated']) && $_REQUEST['updated']) {
			?>
				...
				<p>
				<strong><?php _e( 'Settings saved.', 'googleappslogin-enterprise'); ?></strong>
				</p>
				</div>
			<?php
		}
	...
	protected function ga_mainsection_text() {
		...
		echo sprintf( __( "The <a href='%s'>instructions</a> above will guide you to Google's Cloud Console where you will enter two URLs, and also obtain two codes (Client ID and Client Secret) which you will need to enter in the boxes below.",
			 'googleappslogin-enterprise'), $this->calculate_instructions_url()."#config" );
		echo '</p>';

		$options = $this->get_option_galogin(); // Must be in this order to invoke upgrade code
		$saoptions = $this->get_sa_option();

		echo '<label for="input_ga_clientid" class="textinput big">'.__('Client ID', 'googleappslogin-enterprise').'</label>';
		...
		printf(
			__( 'Normally something like %s', 'googleappslogin-enterprise' ),
			'<code>1234567890123-w1dwn5pfgjeo96c73821dfbof6n4kdhw.apps.googleusercontent.com</code>'
		);

		echo '</p>';

		echo '<label for="input_ga_clientsecret" class="textinput big">'.__('Client Secret', 'googleappslogin-enterprise').'</label>';
		...
		if ( count( $serviceacct_plugins ) == 0 ) { ?>
			...
			<p>
				<?php esc_html_e( 'Some Google Apps extensions may require you to set up a Service Account. If you Activate those extension plugins then come back to this page, you will see further instructions, including the "permission scopes" those extensions require. However, if you know you need to set up a Service Account in advance, you can click below to reveal the settings.', 'googleappslogin-enterprise' ); ?>
			</p>

			<p>
				<a href="#" id="gal-show-admin-serviceacct">
					<?php esc_html_e( 'Show Service Account settings', 'googleappslogin-enterprise' ); ?>
				</a>
			</p>
		...
		if ($saoptions['ga_serviceemail'] != '') {
			if ($saoptions['ga_serviceid'] != '') {
				// Display client id
				echo '<label for="input_ga_serviceid" class="textinput">'.__('Service Account Client ID / Name', 'googleappslogin-enterprise').'</label>';
			...
			echo '<label for="input_ga_serviceemail" class="textinput">'.__('Service Account email address', 'googleappslogin-enterprise').'</label>';
			echo "<div class='gal-lowerinput'>";
			...
			if ($saoptions['ga_pkey_print'] != '') {
				...
				echo '<label for="input_ga_pkey_print" class="textinput">'.__('Private key fingerprint', 'googleappslogin-enterprise').'</label>';
				echo "<div class='gal-lowerinput'>";
		...
		echo '<label for="input_ga_keyfileupload" class="textinput gal_jsonkeyfile">'.__('Upload a new Service Account JSON file', 'googleappslogin-enterprise').'</label>';
		echo '<label for="input_ga_keyjson" class="textinput gal_jsonkeytext" style="display: none;">'.__('Paste contents of JSON file', 'googleappslogin-enterprise').'</label>';
		...
		echo '<label for="input_ga_domainadmin" class="textinput">'.__('A Google Apps Domain admin\'s email', 'googleappslogin-enterprise').'</label>';
	...
	protected function ga_advancedsection_text() {
		...
		echo '<p>';
		printf( __('Once you have the plugin working, you can try these settings to customize the login flow for your users.', 'googleappslogin-enterprise')
				.' '.__('See <a href="%s" target="gainstr">instructions here</a>.', 'googleappslogin-enterprise'),
		...
		_e( 'Force user to confirm Google permissions every time' , 'googleappslogin-enterprise' );
		echo '</label>';
		...
		_e( 'Automatically redirect to Google from login page' , 'googleappslogin-enterprise' );
		echo '</label>';
		...
		_e( 'Remember Me - do not log users out at end of browser session' , 'googleappslogin-enterprise' );
		echo '</label>';
		...
		_e( 'Display \'Powered By wp-glogin.com\' on Login form' , 'googleappslogin-enterprise' );
		echo '</label>';

		echo $this->ga_advancedsection_extra();
		...
		echo '<input id="input_ga_loginbuttontext" name="'.$this->get_options_name().'[ga_loginbuttontext]" size="25" type="text" value="'
						.esc_attr($options['ga_loginbuttontext']).'" class="textinput" maxlength="30" placeholder="'.esc_attr__( 'Login with Google' , 'googleappslogin-enterprise').'" />';
		...
		if (is_multisite()) {
			echo '<h3>'.__( 'Multisite Options' , 'googleappslogin-enterprise').'</h3><p>';
			printf( __('This setting is for multisite admins only. See <a href="%s" target="gainstr">instructions here</a>.', 'googleappslogin-enterprise')
			...
			echo '<label for="input_ga_ms_usesubsitecallback" class="checkbox plain">'.__( 'Use sub-site specific callback from Google' , 'googleappslogin-enterprise').'</label>';
			echo '<br class="clear" />';

			echo '<p class="desc">';
			_e( 'Leave unchecked if in doubt' , 'googleappslogin-enterprise');
			echo '</p>';
		}
	...
	protected function get_error_string($fielderror) {
		$local_error_strings = Array(
				'ga_clientid|tooshort_texterror' => __('The Client ID should be longer than that', 'googleappslogin-enterprise') ,
				'ga_clientsecret|tooshort_texterror' => __('The Client Secret should be longer than that', 'googleappslogin-enterprise'),
				'ga_serviceemail|invalid_email' => __('Service Account email must be a valid email addresses', 'googleappslogin-enterprise'),
				'ga_domainadmin|invalid_email' => __('Google Apps domain admin must be a valid email address of one of your Google Apps admins', 'googleappslogin-enterprise'),
				'ga_jsonkeyfile|file_upload_error' => __('Error with file upload on the server', 'googleappslogin-enterprise'),
				'ga_jsonkeyfile|file_upload_error2' => __('Error with file upload on the server - file was too large', 'googleappslogin-enterprise'),
				'ga_jsonkeyfile|file_upload_error6' => __('Error with file upload on the server - no temp directory exists', 'googleappslogin-enterprise'),
				'ga_jsonkeyfile|file_upload_error7' => __('Error with file upload on the server - failed to write to disk', 'googleappslogin-enterprise'),
				'ga_jsonkeyfile|no_content' => __('JSON key file was empty', 'googleappslogin-enterprise'),
				'ga_jsonkeyfile|decode_error' => __('JSON key file could not be decoded correctly', 'googleappslogin-enterprise'),
				'ga_jsonkeyfile|missing_values' => __('JSON key file does not contain all of client_email, client_id, private_key, and type', 'googleappslogin-enterprise'),
				'ga_jsonkeyfile|not_serviceacct' => __('JSON key file does not represent a Service Account', 'googleappslogin-enterprise'),
				'ga_jsonkeyfile|bad_pem' => __('Key cannot be coerced into a PEM key - invalid format in private_key of JSON key file', 'googleappslogin-enterprise')
		);
		if (isset($local_error_strings[$fielderror])) {
			return $local_error_strings[$fielderror];
		}
		return __( 'Unspecified error' , 'googleappslogin-enterprise');
	}
	...
	protected function get_default_options() {
		return Array('ga_version' => $this->PLUGIN_VERSION,
						...
						'ga_rememberme' => false,
						'ga_loginbuttontext' => 'Login with google',
						'ga_sakey' => '',
						'ga_domainadmin' => '');
	}
	...
	public function ga_plugin_action_links( $links, $file ) {
		if ($file == $this->my_plugin_basename()) {
			$settings_link = '<a href="'.$this->get_settings_url().'">'.__( 'Settings' , 'googleappslogin-enterprise').'</a>';
